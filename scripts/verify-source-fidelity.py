...
from evidence.source_state import capture_source_state, read_source_state
...
def main() -> int:
    ...
    if read_regular(args.preflight / "worktree" / "status.bin"):
        fail("historical preflight checkout status is dirty")
    require_source_state(source_root, args.preflight / "source-state")
...
def require_source_state(source_root: Path, expected_directory: Path) -> None:
    expected_source_state = read_source_state(expected_directory)
    actual_source_state = capture_source_state(source_root)
    for (name, expected), (actual_name, actual) in zip(expected_source_state, actual_source_state, strict=True):
        if name != actual_name or expected != actual:
            fail(f"source-state artifact drift: {name}")
