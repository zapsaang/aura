...
class HistoricalProducerTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        _git(self.protected, "init", "-q", "-b", "main")
        (self.protected / "source.txt").write_text("retained\n", encoding="utf-8")
        _git(self.protected, "add", "source.txt")
        _git(self.protected, "commit", "-q", "-m", "retained")
    ...
    def test_transport_preserves_retained_source_state_bytes(self) -> None:
        expected = {
            "status.bin": b"?? retained\x00",
            "tracked.patch": b"diff --git retained\n\x00",
            "name-status.bin": b"M\x00retained\x00",
            "name-status-no-renames.bin": b"M\x00retained\x00",
        }
        transport = self.retained / "preflight-source-state"
        transport.mkdir()
        for name, raw in expected.items():
            (transport / name).write_bytes(raw)
        _retained_fixture(self.retained, self.base / "absent-source", self.device, self.inode)
        out = self.base / "out"
        out.mkdir()

        prepare._capture_preflight(
            out,
            self.checkout,
            BASELINE,
            self.retained / "source-root.json",
            transport=True,
        )

        for name, raw in expected.items():
            self.assertEqual((out / "source-state" / name).read_bytes(), raw)
        self.assertEqual((out / "worktree" / "status.bin").read_bytes(), b"")
