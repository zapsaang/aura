...
def _capture_preflight(
    ...
    *,
    ...
    source_state = (
        read_source_state(retained.parent / "preflight-source-state")
        if transport
        else capture_source_state(source_root)
    )
    for name, raw in source_state:
        write_new(root / "source-state" / name, raw)
    if transporting:
        append_operation(
            root / "operations.log",
